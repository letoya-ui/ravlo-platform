...
# ✅ Profile model: prefer InvestorProfile
try:
    from LoanMVP.models.investor_models import InvestorProfile
except Exception:
    # fallback if you haven't created investor_models yet
    from LoanMVP.models.loan_models import BorrowerProfile as InvestorProfile
...
# =========================================================
# 📊 INVESTOR TIMELINE (Ravlo Capital Flow)
# =========================================================

INVESTOR_TIMELINE = [
    {"step": 1, "title": "Capital Application Submitted", "key": "application_submitted"},
    {"step": 2, "title": "Documents Verified", "key": "documents_uploaded"},
    {"step": 3, "title": "Deal Review", "key": "processing_review"},
    {"step": 4, "title": "Underwriting Analysis", "key": "underwriting"},
    {"step": 5, "title": "Capital Conditions Issued", "key": "conditions_issued"},
    {"step": 6, "title": "Conditions Cleared", "key": "conditions_cleared"},
    {"step": 7, "title": "Final Capital Approval", "key": "final_approval"},
    {"step": 8, "title": "Funding Scheduled", "key": "closing_scheduled"},
    {"step": 9, "title": "Deal Funded", "key": "loan_closed"},
]
...
@investor_bp.route("/dashboard", methods=["GET"])  # legacy
@login_required
@role_required("investor")
def command_center():
    """
    Investor Command Center — clean investor-only route.
    Uses Ravlo base layout via templates/investor/*.html
    """
    ...
    if ip:
        ...
        # All capital requests (or deals) for this investor
        capital_requests = LoanApplication.query.filter_by(
            investor_profile_id=ip.id
        ).order_by(LoanApplication.created_at.desc()).all()

        # Active capital request
        active_request = LoanApplication.query.filter_by(
            investor_profile_id=ip.id,
            is_active=True
        ).first()
