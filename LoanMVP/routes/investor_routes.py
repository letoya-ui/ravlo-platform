...
def generate_build_blueprint():
    ...
    try:
        ...
        if not blueprint_file and not blueprint_url and not reference_image_url and project:
            blueprint_url = (project.blueprint_url or "").strip()

            if not blueprint_url:
                reference_image_url = (
                    (project.site_plan_url or "").strip()
                    or (project.concept_render_url or "").strip()
                )

        if not blueprint_file and not blueprint_url and not reference_image_url and deal is not None:
            results = deal.results_json or {}
            build_project = results.get("build_project", {}) or {}
            blueprint_block = build_project.get("blueprint", {}) or {}
            exterior_block = build_project.get("exterior", {}) or {}
            ...
            if not blueprint_url:
                reference_image_url = (
                    blueprint_block.get("build_reference_image")
                    or exterior_block.get("image_url")
                    or exterior_block.get("build_reference_image")
                    or ""
                ).strip()
        ...
        if not blueprint_file and not blueprint_url and not reference_image_url:
            return jsonify({
                "status": "error",
                "message": "Provide blueprint_file, blueprint_url, project seed, or reference image."
            }), 400

        # --------------------------------------------------
        # Resolve raw bytes from best available source
        # --------------------------------------------------
        if blueprint_file:
        ...
        # --------------------------------------------------
        # Save uploaded/reference blueprint source
        # --------------------------------------------------
        ...
        # --------------------------------------------------
        # Parse blueprint structure if possible
        # --------------------------------------------------
        try:
        ...
        # --------------------------------------------------
        # Engine payload - BASE64 ONLY for image input
        # --------------------------------------------------
        payload = {
            "image_base64": base64.b64encode(raw).decode("utf-8"),
            "image_url": "",
        ...
        if deal is not None:
            ...
            build_project["blueprint"] = {
                ...
                "build_reference_image": reference_image_url or stored_blueprint_url,
            }
        ...
        return jsonify({
            "status": "error",
            "message": f"Blueprint render failed: {e}"
        }), 500
...
def generate_full_build():
    ...
    try:
        ...
        style_preset = (data.get("style_preset") or "luxury_modern").strip().lower()
        renovation_level = (data.get("renovation_level") or "medium").strip().lower()
        ...
        if blueprint_file:
            ...
            try:
                raw_blueprint = download_image_bytes(reference_image_url)
                if raw_blueprint:
                    blueprint_image_base64 = base64.b64encode(raw_blueprint).decode("utf-8")
            except Exception:
                raw_blueprint = None
            ...
            if getattr(project, "blueprint_url", None):
                blueprint_url = (project.blueprint_url or "").strip()
                try:
                    raw_blueprint = download_image_bytes(blueprint_url)
                    if raw_blueprint:
            ...
            blueprint_url = (saved_blueprint.get("blueprint_url") or saved_blueprint.get("image_url") or "").strip()
        ...
        blueprint_payload = {
            ...
            "lot_size": lot_size,
            "zoning": zoning,
            "image_base64": blueprint_image_base64,
            "image_url": "" if blueprint_image_base64 else blueprint_url,
            "width": 1024,
            "height": 1024,
            "steps": 24,
            "guidance": 6.0,
        ...
        # --------------------------------------------------
        # 2. GENERATE EXTERIOR (BLUEPRINT-FIRST, FALLBACK TO LAND/REF)
        # --------------------------------------------------
        exterior_payload = {
            "mode": "exterior",
            "project_name": project_name,
        ...
        # --------------------------------------------------
        # 3. GENERATE INTERIOR (TEXT-ONLY / NO EXTERIOR ANCHOR)
        # --------------------------------------------------
        interior_payload = {
            "mode": "interior",
            "project_name": project_name,
        ...
        if save_to_deal and deal is not None:
            ...
            build_project["interior"] = {
                "latest": room_entry,
                "rooms": [room_entry] if interior_primary_url else [],
            }
