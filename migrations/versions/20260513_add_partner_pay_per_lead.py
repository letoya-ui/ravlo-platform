...
def upgrade():
    ...
    with op.batch_alter_table('partners', schema=None) as batch_op:
        batch_op.add_column(sa.Column(
            'pay_per_lead_enabled', sa.Boolean(), nullable=True))
        batch_op.add_column(sa.Column(
            'lead_price', sa.Float(), nullable=True))
        batch_op.add_column(sa.Column(
            'stripe_customer_id', sa.String(length=255), nullable=True))
        batch_op.add_column(sa.Column(
            'stripe_payment_method_id', sa.String(length=255), nullable=True))

    # Back-fill and enforce NOT NULL on the boolean
    ...
    with op.batch_alter_table('partners', schema=None) as batch_op:
        batch_op.alter_column(
            'pay_per_lead_enabled',
            existing_type=sa.Boolean(),
            nullable=False,
        )

    # ── New partner_lead_charges table ──────────────────────────
    op.create_table(
        'partner_lead_charges',
        sa.Column('id', sa.Integer(), primary_key=True),
        sa.Column('partner_id', sa.Integer(),
                  sa.ForeignKey('partners.id'), nullable=False),
        sa.Column('connection_request_id', sa.Integer(),
                  sa.ForeignKey('partner_connection_requests.id'), nullable=True),
